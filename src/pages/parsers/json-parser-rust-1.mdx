...
1. Introduction, Setup, and Lexer
2. Parser
...
In programming, "parsing" is transforming less-structured input into more-structured output by applying knowledge of the expected shape(s) of the data. A "parser" is a function that does this. This transformation from less-structured to more-structured might fail if the input isn't compatible with the desired output.
...
Now that `lex.rs` has been declared a module, we can write code in it. We'll add the first few "punctuation" variants for `Token`.
...
Finally, we can create a variant for the "end of file". This is not strictly necessary, but it will help the parser code later, when the parser loops through the tokens it can look for this [sentinel value](https://en.wikipedia.org/wiki/Sentinel_value).
...
What does the JSON specification say about integers? What are the tradeoffs of doing so?
...
TODO: image of function with "run test"
...
Whether you use Test Driven Development (TDD) principles or not, it's generally still a good idea to make sure a test fails before it succeeds - to avoid having tests that pass when they shouldn't.
...
As stated in the previous section on modules, the programmer explicitly defines the module tree, it is not implicit based on the filesystem. Also, the parent module defines its child module(s). In this case, the `lex` module defines a child module named `tests`.
...
The `super` keyword when used in a path refers to the parent module. In this case, the current module is `lex::tests` and the parent module of that is `lex`.
...
I generally follow the "AAA" test style, which is Arrange-Act-Assert. - Arrange: set up the input data and expected result - Act: run the function - Assert: check the result

</EmphasisBox>

## Implementing the Lexer

Time to actually implement the lexer!
...
`usize` is an "unsigned integer, with a size based on your machine". Specifically, the size is equivalent to the size to reference something in memory, sometimes called the "pointer size". For example, on a 32-bit platform this is 32 bits and on a 64-bit platform this is 64 bits. If we have 128-bit platforms in the future, this would be 128 bits.

</NewConcept>

### Finally, make a Token

Add two lines into the `lex` function, and create a new function.
...
There are a few ways to go about this. One way is to create a [Finite-State Machine](https://en.wikipedia.org/wiki/Finite-state_machine) that tracks states such as "Is Lexing Null Token" and looks for the appropriate letters. You'd also need to keep track of which letters have been processed so far.
...
Consider the data we have so far. We have `chars` and `index` which are owned by the `lex` function. The `make_token` function needs this data so that it can crunch through the characters that it needs to create the next token. Let's see how that concept translates to Rust.
...
TODO: Drake meme (sharing Yes, mutating Yes, shared mutation No)
...
Let's make the code compile again, we can [dereference]() the index and get the character at that index.
...
Let's add a function that looks for the characters "n", "u", "l", "l" to lex the literal `null` into a `Token`.
...
fn lex_null(chars: &Vec<char>, index: &mut usize) -> Token {
...
If we try to lex the following (not) JSON:
...
4. In this particular case of a JSON parser, bad data is not exceptional at all. **Any time** that data is ingested from outside the system, you need to assume that the data might be bad. This is not exceptional, thus an Exception should not be used.
...
```rust
/// One of the possible errors that could occur while lexing the input
...
fn lex_null(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
    for expected_char in "null".chars() {
        if expected_char != chars[*index] {
            return Err(LexError::UnfinishedLiteralValue);
        }
        *index += 1;
    }
    Ok(Token::Null)
...
First, we defined an `enum` for ourselves to define the possible error variants. Second, we updated the `lex_null` function to return a `Result` containing a `Token` (when `Ok`) or containing a `LexError` (when `Err`).

Now that `lex_null` can fail, so can `make_token`.

```rust
fn make_token(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
    let ch = chars[*index];

    let token = match ch {
        // ...snip...

        'n' => match lex_null(chars, index) {
            Ok(token) => token,
...
2. We `match` on the Result of the `lex_null` function, because it can either succeed or fail. If it fails, we go ahead and `return` that error, otherwise we grab the `Token` from inside the `Result`.
3. At the bottom of the function, we return the `Token` inside of the `Ok` variant.

<NewConcept title="question mark operator (?)">

The `match` expression we just added in the previous step is a bit wordy. You can imagine we would have to do the same thing for any lexing that can fail, which will get repetitive and verbose. We can replace `match` expressions like that one with the `?` operator.

This `?` operator creates an "Error Propagation Expression". This expression "passes up" (i.e. returns) the error if it's an `Err`, otherwise it unwraps the `Ok` value. We can now replace that `match` expression with the `?` operator:
...
We can now write the other functions for lexing the `true` and `false` literal values.
...
fn make_token(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
    ...
    let token = match ch {
        ...
        'f' => lex_false(chars, index)?,
        't' => lex_true(chars, index)?,
...
fn lex_false(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
...
fn lex_true(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
...
The `lex_null`, `lex_false`, and `lex_true` all look quite similar. The only differences are the literal string being compared and the variant of the `Token` that is returned.

How would you remove the duplication here? What are the tradeoffs of doing this?
...
TODO: fix bug of incrementing index too much
...
fn make_token(chars: &Vec<char>, index: &mut usize) -> Result<Token, LexError> {
    ...
    let token = match ch {
        ...
        c if c.is_ascii_digit() => lex_float(chars, index)?,
...
We've introduced a new function, `lex_float`. Time to implement this:

```rust
fn lex_float(chars: &Vec<char>, curr_idx: &mut usize) -> Result<Token, LexError> {
    ...
    match unparsed_num.parse() {
        ...
        Err(err) => Err(LexError::ParseNumberError(err)),
    }
}
```
...
Time for the last kind of Token, and we are almost done!
...
```rust
fn lex_string(chars: &[char], curr_idx: &mut usize) -> Result<Token, LexError> {
...
3. Create a token from the lexed string and return it
...
fn just_ken() {
    ...
    let expected = [Token::String("ken".to_string())];
...
However, we're not covering all of the cases! What should happen if the JSON document looked like this?
...
That involves adding a new variant to our `LexError`:
...
We'll need to rework the main loop of `lex_string`. Instead of breaking the loop when the index reaches the end of the input string, which naturally happened in the `while` loop, this case needs to be an error instead.
    ...
    loop {
        *curr_idx += 1;
        if *curr_idx >= chars.len() {
            return Err(LexError::UnclosedQuotes);
        }
        let ch = chars[*curr_idx];
        if ch == '"' {
            break;
...
Finally, a test for an unclosed quote. Note that tests for error conditions still follow the Arrange-Act-Assert format, because errors are just values too (this might be obvious to some readers, but it wasn't obvious to me at first, coming from languages where you have to test with assertions like `expect(foo).toThrow()`).

```rust
#[test]
fn unclosed_string() {
    let input = String::from("\"unclosed");
    let expected = Err(LexError::UnclosedQuotes);
...
We have now completed _an_ implementation of a lexer for JSON tokens. This series continues in Part 2: Writing the Parser.
