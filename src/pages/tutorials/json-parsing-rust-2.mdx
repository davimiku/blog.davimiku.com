export const meta = {
    ...
    seriesName: 'JSON Parsing (Rust)',
    seriesSlug: 'json-parsing-rust',
    seriesIndex: 2,
    readingTime: 30,
}

import { EmphasisBox, Figure, KnowledgeCheck, NewConcept, Spoiler } from '../../components/blog'
...
<NewConcept title="slice">

In [Part 1](./json-parsing-rust-1), there was a remark that instead of borrowing a Vector using `&Vec<T>`, it was more idiomatic to use a [slice](https://doc.rust-lang.org/book/ch04-03-slices.html), which would be written like `&[T]`.

Using a slice rather than a borrowed `Vec` gives more flexibility to callers of that function - there are many ways to create a slice, not just from a `Vec`.

</NewConcept>

This function uses the same kind of signature as our previous work from Part 1, it borrows the tokens and produces either a JSON value or an error.
...
<Figure src='https://www.json.org/img/value.png' alt='flow diagram for possible JSON values' />
...
<NewConcept title="array">

The square bracket syntax above is an [array](https://doc.rust-lang.org/book/ch03-02-data-types.html#the-array-type).

```rust
let input = [Token::Null];
```

Note that a Rust array is **fixed-length**, meaning that elements cannot be added or removed. Mutability of the array follows the same rules as the rest of Rust, meaning that the owner of the array can mutate it by reassigning an element or mutably borrowing an element. If an entity is not the owner of the array, then it would need to have an exclusive reference (`&mut`) in order to mutate it.

Recall the note about why slices were preferred as function parameters before, this is an example of `parse_tokens` needing a slice, and in different places we've been able to provide that from either a `Vec` or an array.
...
Similarly to how a `Vec<T>` can be borrowed as a slice `&[T]`, rather than borrowing a `String` with `&String`, it's more idiomatic to use a _string slice_, which is written as `&str`.
...
The `X` is any hexadecimal character, so 0-9 and A-F.

<KnowledgeCheck>

1.) Are the 6 letters included in hexadecimal case sensitive? i.e. are both a-f and A-F acceptable?

2.) What does the specification say about JSON strings that use a backslash to "escape" a letter that doesn't need to be escaped?
...
In any case, when parsing if we see `\/`, `\p`, or `\q`, we would basically ignore the backslash and just store `/`, `p`, or `q` (respectively). This goes for any character not listed in the table above, which is why I did not list the solidus there, even though it is listed in the RFC.
...
-- surrogate pairs \​uD83C\​uDF3C is *not* a valid escape
-- in a Python3 string literal
```
...
In some cases, `if let` can be more concise. Another thing to consider with readability is _intent_, the `if let` syntax shows that one variant is more "important" than the others, whereas in a `match`, all the variants are roughly "equal", so to speak.
...
export default function JsonParsingRust2({ children }) {
