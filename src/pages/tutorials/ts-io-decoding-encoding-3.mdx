...
<MarginNote id='unused-param-eslint-warnings'>
  I used underscores for the unused parameters to silence ESLint warnings.
</MarginNote>
Now that we have this, we can properly type the encoding of an array!

```typescript
interface ShapeArray<S extends BaseShape> extends Shape<Infer<S>[], unknown, OutputOf<S>[]> {
  //                               ^^^^^^^^^^^^^
  readonly __tag: 'array'
}
```
...
**Should** it be optional? How would that change how users interact with and use our library, does it make it simpler/easier or more complex/difficult?
