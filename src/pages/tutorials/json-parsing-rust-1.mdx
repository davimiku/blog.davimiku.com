...
Until JavaScript's 2019 edition (ES2019), JSON syntax was _almost_ a subset of JavaScript syntax. There was one edge case -- JSON strings could contain `U+2028` (LINE SEPARATOR) and `U+2029` (PARAGRAPH SEPARATOR) unescaped while in JavaScript they had to be escaped.
...
<span class='sidenote'> A "parser" is a function that does "parsing"</span>This transformation from less-structured
...
TODO: Kombucha girl meme (sharing Yes, mutating Yes, shared mutation No)
...
First, we defined an `enum` for ourselves to define the possible error variants. Second, we updated the `tokenize_null` function to return a `Result` containing a `Token` (when `Ok`) or containing a `TokenizeError` (when `Err`).
...
Are there any other cases for numbers from [JSON.org](https://www.json.org) that we missed? Hint: what does "E" mean in a number?
...
That involves adding a new variant to our `TokenizeError`:
